...
export const EventDetailPage = () => {
  ...
  const bookedTickets = event?.bookings?.reduce((sum: number, booking: Booking) => sum + (booking.quantity || 0), 0) || 0;
  ...
  return (
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <Card>
        ...
        {((user?.role === UserRole.ADMIN) || (user?.role === UserRole.EVENT_MANAGER && user?.id === event?.creator?.id)) && (
          ...
          <div className="space-y-2">
            ...
            <p>
              ...
              {event?.bookings && event.bookings.length > 0 && (
                isFull ? (
                  <span className="ml-2 font-semibold text-destructive">(Full)</span>
                ) : (
            ...
            {event?.creator && (
              <p>
                Event Manager: {event.creator.first_name} {event.creator.last_name}
              </p>
            )}
          </div>
                  ...
                  onSubmit={async (values) => {
                    try {
                      await createBooking.mutateAsync({
                        ...
                        booking_amount:
                          Number(event?.ticket_price ?? 0) * values.quantity,
                      });
                      navigate("/bookings");
                    } catch {
                      return;
            ...
            {event?.bookings?.length ? (
              <div className="space-y-3">
                {event.bookings.map((b: Booking, idx: number) => (
                  <div key={b.id ?? `booking-${idx}`} className="p-3 border rounded">
